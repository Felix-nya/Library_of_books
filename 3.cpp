...
struct Node {
	...
	int date;
	Node* next;
	Node(string aut, string name, int date) : author(aut), name(name), date(date), next(NULL) {}
};

typedef Node* PNode;

PNode CreateNode(string aut, string name, int date)
{
	return new Node(aut, name, date);
}
...
void showList(int cursor, PNode& Head) {
	...
	while (q) {
		if (n == cursor) {
			cout << setw(2) << left << "=>" << setw(4) << left << n << setw(30) << left << q->author << setw(50) << left << q->name << setw(30) << left << q->date << endl;
		}
		else {
			cout << setw(4) << left << n << setw(30) << left << q->author << setw(50) << left << q->name << setw(30) << left << q->date << endl;
		}
		q = q->next;
		n++;
	}
...
void Interface(PNode& Head) {
	...
	gotoxy(0, 6);
	cout << setw(4) << left << "№" << setw(30) << left << "Автор" << setw(50) << left << "Название" << setw(30) << left << "Год издания";
	gotoxy(0, 7);
	for (int i = 0; i < Console_wide(); i++) cout << "_";
	gotoxy(0, 8);
	showList(0, Head);
...
void Programm(PNode& Head) {
	...
	do {
		...
		if (ch == 'a' || ch == 'A' || ch == 'ф' || ch == 'Ф') {
			...
			dat = stoi(datstr);
			AddLast(CreateNode(aut, nam, dat), Head);
			position_curs++;
		}
