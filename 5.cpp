	...
	do {
		...
		if (ch == 'a' || ch == 'A' || ch == 'ф' || ch == 'Ф') {
			...
			while (getline(File, line_in_file)) {
				int pos1 = line_in_file.find(';');
				int pos2 = line_in_file.find(';', pos1 + 1);
				int pos3 = line_in_file.find(';', pos2 + 1);

				string aut = line_in_file.substr(0, pos1);
				string nam = line_in_file.substr(pos1 + 1, pos2 - pos1 - 1);
				int dat = stoi(line_in_file.substr(pos2 + 1, pos3 - pos2 - 1));
				string lists = line_in_file.substr(pos3 + 1);
